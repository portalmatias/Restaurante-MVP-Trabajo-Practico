...
export class ReservasService {
   ...
   * (capacidad de mesa), 3 (turno activo y mesa de la zona solicitada) y 4 (aforo de zona) —
   * ver design.md → "Invariantes 2, 3 y 4 → validación de servicio". El invariante 1
   * (exclusividad mesa+turno+fecha) lo hace cumplir el índice único parcial de la base; acá
   * se traduce el `P2002` de Prisma a un `409 Conflict` legible en vez de propagarlo crudo.
   */
  async crearReserva(input: CrearReservaInput) {
    return this.prisma.$transaction(async (tx) => {
      ...
      for (let intento = 0; intento < CODIGO_RESERVA_MAX_INTENTOS; intento++) {
