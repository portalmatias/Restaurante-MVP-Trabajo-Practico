...
- [x] 1.1 Confirmar que el PR #12 (`modelo-dominio`) está mergeado a `main` y crear
...
- [x] 1.2 Confirmar que `backend/src/common/timezone.ts` existe en `main` con
...
- [x] 1.3 Si ni #12 ni `auth-admin` los agregaron, sumar `class-validator` y
...
- [x] 1.4 Crear `backend/src/disponibilidad/disponibilidad.module.ts`, vacío por ahora, y
...
- [x] 2.1 Escribir `backend/src/common/timezone.spec.ts` **antes** de la
...
- [x] 2.2 Implementar `inicioTurnoUtc(fecha, horaInicio)` en `backend/src/common/timezone.ts`
...
- [x] 3.1 Definir los tipos en `backend/src/disponibilidad/reglas/`: `CodigoMotivo` (enum con
...
- [x] 3.2 Escribir `evaluar-reglas.spec.ts` para turno y anticipación, con contextos armados a
...
- [x] 3.3 Sumar a `evaluar-reglas.spec.ts` los casos de comensales, aforo y mesa: bordes 1/8
...
- [x] 3.4 Sumar los casos de orden, lugares restantes y medianoche: los cuatro motivos a la vez
...
- [x] 3.5 Implementar `evaluarReglas` y `calcularLugaresRestantes` como funciones puras (D1,
...
- [x] 4.1 Escribir `backend/test/disponibilidad-contexto.e2e-spec.ts` **antes** del cargador,
...
- [x] 4.2 Implementar `cargarContexto(db, solicitud)` con la API tipada de Prisma (D1). La
...
- [x] 4.3 Implementar `bloquearTurnoFecha(tx, turnoId, fecha)` con `$executeRaw` y un
...
- [x] 5.1 Implementar `DisponibilidadService.consultar(query)`: convertir `fecha` con
...
- [x] 6.2 Crear los DTOs y el controller decorado, con el cuerpo del handler sin implementar:
      ...
      sin diferencias.
- [x] 6.3 Implementar el handler delegando en `DisponibilidadService.consultar`, sin lógica en
...
- [x] 7.1 Crear `backend/test/disponibilidad.e2e-spec.ts` levantando la app con el mismo
      ...
      termina.
- [x] 7.2 Tests `400`: falta `zonaId`; `fecha=2026-09-19T20:00:00Z`; `fecha=2026-02-30`;
...
- [x] 7.3 Tests `404`: `turnoId` UUID inexistente y `zonaId` UUID inexistente, con cuerpo
      `{ statusCode: 404, message: string, error: "Not Found" }`. Verificar con el mismo
      comando.
- [x] 7.4 Tests `200`: consulta sin header `Authorization` con lugar (`disponible: true`,
...
- [x] 8.1 Escribir `backend/test/disponibilidad-lock.e2e-spec.ts` con un helper de test que
...
- [x] 8.2 Hasta que `ci-integracion-db` esté mergeado, correr las suites de las secciones 4,
