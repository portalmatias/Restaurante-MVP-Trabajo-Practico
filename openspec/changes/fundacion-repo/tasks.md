...
- [x] 2.3 Escribir el esqueleto de `openapi/openapi.yaml`: `openapi: 3.1.0`, `info` con título y descripción **en español** (§8), `servers` apuntando al backend local, `components.securitySchemes` con bearer JWT según §5, y los `paths` que el backend realmente exponga. Verificar que un parser de OpenAPI 3.1 lo carga sin errores. *Quedó documentado `GET /`, el endpoint de estado del scaffolding: el chequeo de deriva de 2.5 lo detectó y el contrato tiene que reflejar lo que el backend expone (ver D2).*
...
- [x] 2.5 Implementar `openapi:check` (D1/D2): generar el spec desde `@nestjs/swagger` y diffear **solo** `paths` y `components.schemas` contra el YAML commiteado. Verificar que se pone en rojo cuando el backend expone un endpoint que no está en el YAML. *Se puso rojo solo, en su primera corrida, con el `GET /` del scaffolding.*
