...
- [x] 1.1 Confirmar que la implementación de `fundacion-repo` está mergeada a `main` y que
...
- [x] 1.2 Confirmar que la implementación de `modelo-dominio` ya aplicó la migración que crea
...
- [x] 2.1 Crear `backend/src/auth/` (`auth.module.ts`, `dto/login.dto.ts` con
...
- [x] 2.2 Implementar `AuthService.login`: busca el `Usuario` por email, compara la
...
- [x] 2.3 Implementar `AuthController` con `POST /auth/login` que llama a
...
- [x] 2.4 Aplicar `@Throttle()` sobre `POST /auth/login` con un límite más estricto que el
...
- [x] 3.1 Implementar `JwtStrategy` (`passport-jwt`) que valida firma y expiración contra
      `JWT_SECRET` y devuelve `{ id, rol }`. Verificar con un test unitario: un token válido
      pasa, uno con firma inválida es rechazado.
- [x] 3.2 Implementar `JwtAuthGuard` (extiende `AuthGuard('jwt')`) y aplicarlo a una ruta de
      prueba. Verificar manualmente (o con Supertest) que una request sin header
      `Authorization` es rechazada.
- [x] 3.3 Implementar el decorador `@Roles()` y `RolesGuard` (con `Reflector`), aplicado
...
- [x] 4.1 Test: login exitoso devuelve un JWT válido (spec: "Login de administrador",
      escenario "Login exitoso devuelve un JWT").
- [x] 4.2 Test: login con email inexistente o contraseña incorrecta devuelve un error
      genérico, sin distinguir cuál dato falló (escenario "Credenciales incorrectas
      rechazadas").
- [x] 4.3 Test: un JWT ya expirado es rechazado en una ruta protegida (spec: "Contenido y
      vigencia del JWT", generando un token con expiración forzada a un instante pasado).
- [x] 4.4 Test e2e: una ruta de admin sin header `Authorization` responde `401` (spec:
      "Autenticación exigida en rutas de admin" — también exigido explícitamente por
      `config.yaml` §9).
- [x] 4.5 Test e2e: una ruta de admin con JWT válido pero de un rol distinto al requerido
      responde `403` (spec: "Autorización por rol en rutas de admin" — también exigido
      explícitamente por `config.yaml` §9).
- [x] 4.6 Test e2e: superar el límite de intentos configurado contra `/auth/login` responde
      `429` en el intento siguiente, sin llegar a validar credenciales (spec: "Límite de
      intentos de login").

## 5. Verificación final

- [x] 5.1 Correr `openspec validate auth-admin --strict` y confirmar que el change es válido.
...
- [x] 5.3 Confirmar que no hicieron falta variables de entorno nuevas (`JWT_SECRET` y
      ...
      el mismo PR. No hizo falta ninguna variable nueva; de paso se corrigió que
      `JWT_EXPIRES_IN` estaba documentada pero no se leía (el módulo tenía `expiresIn`
      hardcodeado en `'60m'`).
